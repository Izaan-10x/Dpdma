// ==============================================================
// Generated by Vitis HLS v2024.2
// Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
// Copyright 2022-2024 Advanced Micro Devices, Inc. All Rights Reserved.
// ==============================================================

`timescale 1 ns / 1 ps 

module bd_quad_vmix_dp_v_tpg_3_0_v_tpgHlsDataFlow (
        height_val4,
        width_val7,
        field_id_val8,
        fid_in_val9,
        bckgndId_val10,
        ovrlayId_val11,
        maskId_val12,
        motionSpeed_val14,
        colorFormat_val17,
        crossHairX_val18,
        crossHairY_val19,
        boxSize_val20,
        boxColorR_val21,
        boxColorG_val22,
        boxColorB_val23,
        m_axis_video_TDATA,
        m_axis_video_TKEEP,
        m_axis_video_TSTRB,
        m_axis_video_TUSER,
        m_axis_video_TLAST,
        m_axis_video_TID,
        m_axis_video_TDEST,
        fid,
        ap_clk,
        ap_rst,
        field_id_val8_ap_vld,
        fid_in_val9_ap_vld,
        ovrlayId_val11_ap_vld,
        maskId_val12_ap_vld,
        crossHairX_val18_ap_vld,
        crossHairY_val19_ap_vld,
        boxSize_val20_ap_vld,
        boxColorR_val21_ap_vld,
        boxColorG_val22_ap_vld,
        boxColorB_val23_ap_vld,
        ap_start,
        height_val4_ap_vld,
        width_val7_ap_vld,
        bckgndId_val10_ap_vld,
        motionSpeed_val14_ap_vld,
        colorFormat_val17_ap_vld,
        m_axis_video_TVALID,
        m_axis_video_TREADY,
        fid_ap_vld,
        ap_done,
        ap_ready,
        ap_idle,
        ap_continue
);


input  [15:0] height_val4;
input  [15:0] width_val7;
input  [15:0] field_id_val8;
input  [0:0] fid_in_val9;
input  [7:0] bckgndId_val10;
input  [7:0] ovrlayId_val11;
input  [7:0] maskId_val12;
input  [7:0] motionSpeed_val14;
input  [7:0] colorFormat_val17;
input  [15:0] crossHairX_val18;
input  [15:0] crossHairY_val19;
input  [15:0] boxSize_val20;
input  [7:0] boxColorR_val21;
input  [7:0] boxColorG_val22;
input  [7:0] boxColorB_val23;
output  [47:0] m_axis_video_TDATA;
output  [5:0] m_axis_video_TKEEP;
output  [5:0] m_axis_video_TSTRB;
output  [0:0] m_axis_video_TUSER;
output  [0:0] m_axis_video_TLAST;
output  [0:0] m_axis_video_TID;
output  [0:0] m_axis_video_TDEST;
output  [0:0] fid;
input   ap_clk;
input   ap_rst;
input   field_id_val8_ap_vld;
input   fid_in_val9_ap_vld;
input   ovrlayId_val11_ap_vld;
input   maskId_val12_ap_vld;
input   crossHairX_val18_ap_vld;
input   crossHairY_val19_ap_vld;
input   boxSize_val20_ap_vld;
input   boxColorR_val21_ap_vld;
input   boxColorG_val22_ap_vld;
input   boxColorB_val23_ap_vld;
input   ap_start;
input   height_val4_ap_vld;
input   width_val7_ap_vld;
input   bckgndId_val10_ap_vld;
input   motionSpeed_val14_ap_vld;
input   colorFormat_val17_ap_vld;
output   m_axis_video_TVALID;
input   m_axis_video_TREADY;
output   fid_ap_vld;
output   ap_done;
output   ap_ready;
output   ap_idle;
input   ap_continue;

wire    entry_proc_U0_ap_start;
wire    entry_proc_U0_start_full_n;
wire    entry_proc_U0_ap_done;
wire    entry_proc_U0_ap_continue;
wire    entry_proc_U0_ap_idle;
wire    entry_proc_U0_ap_ready;
wire    entry_proc_U0_start_out;
wire    entry_proc_U0_start_write;
wire   [15:0] entry_proc_U0_field_id_val8_c_din;
wire    entry_proc_U0_field_id_val8_c_write;
wire   [0:0] entry_proc_U0_fid_in_val9_c_din;
wire    entry_proc_U0_fid_in_val9_c_write;
wire   [7:0] entry_proc_U0_ovrlayId_val11_c_din;
wire    entry_proc_U0_ovrlayId_val11_c_write;
wire   [7:0] entry_proc_U0_maskId_val12_c_din;
wire    entry_proc_U0_maskId_val12_c_write;
wire   [15:0] entry_proc_U0_crossHairX_val18_c_din;
wire    entry_proc_U0_crossHairX_val18_c_write;
wire   [15:0] entry_proc_U0_crossHairY_val19_c_din;
wire    entry_proc_U0_crossHairY_val19_c_write;
wire   [15:0] entry_proc_U0_boxSize_val20_c_din;
wire    entry_proc_U0_boxSize_val20_c_write;
wire   [7:0] entry_proc_U0_boxColorR_val21_c_din;
wire    entry_proc_U0_boxColorR_val21_c_write;
wire   [7:0] entry_proc_U0_boxColorG_val22_c_din;
wire    entry_proc_U0_boxColorG_val22_c_write;
wire   [7:0] entry_proc_U0_boxColorB_val23_c_din;
wire    entry_proc_U0_boxColorB_val23_c_write;
wire    tpgBackground_U0_ap_start;
wire    tpgBackground_U0_ap_done;
wire    tpgBackground_U0_ap_continue;
wire    tpgBackground_U0_ap_idle;
wire    tpgBackground_U0_ap_ready;
wire   [47:0] tpgBackground_U0_bckgndYUV_din;
wire    tpgBackground_U0_bckgndYUV_write;
wire   [31:0] tpgBackground_U0_bckgndYUV_num_data_valid;
wire   [31:0] tpgBackground_U0_bckgndYUV_fifo_cap;
wire   [15:0] tpgBackground_U0_height_val4_c3_din;
wire    tpgBackground_U0_height_val4_c3_write;
wire   [15:0] tpgBackground_U0_width_val7_c4_din;
wire    tpgBackground_U0_width_val7_c4_write;
wire   [7:0] tpgBackground_U0_motionSpeed_val14_c_din;
wire    tpgBackground_U0_motionSpeed_val14_c_write;
wire   [7:0] tpgBackground_U0_colorFormat_val17_c5_din;
wire    tpgBackground_U0_colorFormat_val17_c5_write;
wire    tpgForeground_U0_ap_start;
wire    tpgForeground_U0_ap_done;
wire    tpgForeground_U0_ap_continue;
wire    tpgForeground_U0_ap_idle;
wire    tpgForeground_U0_ap_ready;
wire    tpgForeground_U0_bckgndYUV_read;
wire    tpgForeground_U0_height_val_read;
wire    tpgForeground_U0_width_val_read;
wire    tpgForeground_U0_patternId_val_read;
wire    tpgForeground_U0_maskId_val_read;
wire    tpgForeground_U0_colorFormat_val_read;
wire    tpgForeground_U0_crossHairX_val_read;
wire    tpgForeground_U0_crossHairY_val_read;
wire    tpgForeground_U0_boxSize_val_read;
wire    tpgForeground_U0_boxColorR_val_read;
wire    tpgForeground_U0_boxColorG_val_read;
wire    tpgForeground_U0_boxColorB_val_read;
wire    tpgForeground_U0_motionSpeed_val_read;
wire   [47:0] tpgForeground_U0_ovrlayYUV_din;
wire    tpgForeground_U0_ovrlayYUV_write;
wire   [31:0] tpgForeground_U0_ovrlayYUV_num_data_valid;
wire   [31:0] tpgForeground_U0_ovrlayYUV_fifo_cap;
wire   [10:0] tpgForeground_U0_height_val4_c_din;
wire    tpgForeground_U0_height_val4_c_write;
wire   [10:0] tpgForeground_U0_width_val7_c_din;
wire    tpgForeground_U0_width_val7_c_write;
wire   [7:0] tpgForeground_U0_colorFormat_val17_c_din;
wire    tpgForeground_U0_colorFormat_val17_c_write;
wire    MultiPixStream2AXIvideo_U0_ap_start;
wire    MultiPixStream2AXIvideo_U0_ap_done;
wire    MultiPixStream2AXIvideo_U0_ap_continue;
wire    MultiPixStream2AXIvideo_U0_ap_idle;
wire    MultiPixStream2AXIvideo_U0_ap_ready;
wire    MultiPixStream2AXIvideo_U0_ovrlayYUV_read;
wire   [47:0] MultiPixStream2AXIvideo_U0_m_axis_video_TDATA;
wire    MultiPixStream2AXIvideo_U0_m_axis_video_TVALID;
wire   [5:0] MultiPixStream2AXIvideo_U0_m_axis_video_TKEEP;
wire   [5:0] MultiPixStream2AXIvideo_U0_m_axis_video_TSTRB;
wire   [0:0] MultiPixStream2AXIvideo_U0_m_axis_video_TUSER;
wire   [0:0] MultiPixStream2AXIvideo_U0_m_axis_video_TLAST;
wire   [0:0] MultiPixStream2AXIvideo_U0_m_axis_video_TID;
wire   [0:0] MultiPixStream2AXIvideo_U0_m_axis_video_TDEST;
wire    MultiPixStream2AXIvideo_U0_height_val4_read;
wire    MultiPixStream2AXIvideo_U0_width_val7_read;
wire    MultiPixStream2AXIvideo_U0_colorFormat_val17_read;
wire    MultiPixStream2AXIvideo_U0_fid_in_val9_read;
wire   [0:0] MultiPixStream2AXIvideo_U0_fid;
wire    MultiPixStream2AXIvideo_U0_fid_ap_vld;
wire    MultiPixStream2AXIvideo_U0_field_id_val8_read;
wire    field_id_val8_c_full_n;
wire   [15:0] field_id_val8_c_dout;
wire    field_id_val8_c_empty_n;
wire   [2:0] field_id_val8_c_num_data_valid;
wire   [2:0] field_id_val8_c_fifo_cap;
wire    fid_in_val9_c_full_n;
wire   [0:0] fid_in_val9_c_dout;
wire    fid_in_val9_c_empty_n;
wire   [2:0] fid_in_val9_c_num_data_valid;
wire   [2:0] fid_in_val9_c_fifo_cap;
wire    ovrlayId_val11_c_full_n;
wire   [7:0] ovrlayId_val11_c_dout;
wire    ovrlayId_val11_c_empty_n;
wire   [2:0] ovrlayId_val11_c_num_data_valid;
wire   [2:0] ovrlayId_val11_c_fifo_cap;
wire    maskId_val12_c_full_n;
wire   [7:0] maskId_val12_c_dout;
wire    maskId_val12_c_empty_n;
wire   [2:0] maskId_val12_c_num_data_valid;
wire   [2:0] maskId_val12_c_fifo_cap;
wire    crossHairX_val18_c_full_n;
wire   [15:0] crossHairX_val18_c_dout;
wire    crossHairX_val18_c_empty_n;
wire   [2:0] crossHairX_val18_c_num_data_valid;
wire   [2:0] crossHairX_val18_c_fifo_cap;
wire    crossHairY_val19_c_full_n;
wire   [15:0] crossHairY_val19_c_dout;
wire    crossHairY_val19_c_empty_n;
wire   [2:0] crossHairY_val19_c_num_data_valid;
wire   [2:0] crossHairY_val19_c_fifo_cap;
wire    boxSize_val20_c_full_n;
wire   [15:0] boxSize_val20_c_dout;
wire    boxSize_val20_c_empty_n;
wire   [2:0] boxSize_val20_c_num_data_valid;
wire   [2:0] boxSize_val20_c_fifo_cap;
wire    boxColorR_val21_c_full_n;
wire   [7:0] boxColorR_val21_c_dout;
wire    boxColorR_val21_c_empty_n;
wire   [2:0] boxColorR_val21_c_num_data_valid;
wire   [2:0] boxColorR_val21_c_fifo_cap;
wire    boxColorG_val22_c_full_n;
wire   [7:0] boxColorG_val22_c_dout;
wire    boxColorG_val22_c_empty_n;
wire   [2:0] boxColorG_val22_c_num_data_valid;
wire   [2:0] boxColorG_val22_c_fifo_cap;
wire    boxColorB_val23_c_full_n;
wire   [7:0] boxColorB_val23_c_dout;
wire    boxColorB_val23_c_empty_n;
wire   [2:0] boxColorB_val23_c_num_data_valid;
wire   [2:0] boxColorB_val23_c_fifo_cap;
wire    bckgndYUV_full_n;
wire   [47:0] bckgndYUV_dout;
wire    bckgndYUV_empty_n;
wire   [4:0] bckgndYUV_num_data_valid;
wire   [4:0] bckgndYUV_fifo_cap;
wire    height_val4_c3_full_n;
wire   [15:0] height_val4_c3_dout;
wire    height_val4_c3_empty_n;
wire   [2:0] height_val4_c3_num_data_valid;
wire   [2:0] height_val4_c3_fifo_cap;
wire    width_val7_c4_full_n;
wire   [15:0] width_val7_c4_dout;
wire    width_val7_c4_empty_n;
wire   [2:0] width_val7_c4_num_data_valid;
wire   [2:0] width_val7_c4_fifo_cap;
wire    motionSpeed_val14_c_full_n;
wire   [7:0] motionSpeed_val14_c_dout;
wire    motionSpeed_val14_c_empty_n;
wire   [2:0] motionSpeed_val14_c_num_data_valid;
wire   [2:0] motionSpeed_val14_c_fifo_cap;
wire    colorFormat_val17_c5_full_n;
wire   [7:0] colorFormat_val17_c5_dout;
wire    colorFormat_val17_c5_empty_n;
wire   [2:0] colorFormat_val17_c5_num_data_valid;
wire   [2:0] colorFormat_val17_c5_fifo_cap;
wire    ovrlayYUV_full_n;
wire   [47:0] ovrlayYUV_dout;
wire    ovrlayYUV_empty_n;
wire   [4:0] ovrlayYUV_num_data_valid;
wire   [4:0] ovrlayYUV_fifo_cap;
wire    height_val4_c_full_n;
wire   [10:0] height_val4_c_dout;
wire    height_val4_c_empty_n;
wire   [2:0] height_val4_c_num_data_valid;
wire   [2:0] height_val4_c_fifo_cap;
wire    width_val7_c_full_n;
wire   [10:0] width_val7_c_dout;
wire    width_val7_c_empty_n;
wire   [2:0] width_val7_c_num_data_valid;
wire   [2:0] width_val7_c_fifo_cap;
wire    colorFormat_val17_c_full_n;
wire   [7:0] colorFormat_val17_c_dout;
wire    colorFormat_val17_c_empty_n;
wire   [2:0] colorFormat_val17_c_num_data_valid;
wire   [2:0] colorFormat_val17_c_fifo_cap;
wire    ap_sync_ready;
reg    ap_sync_reg_entry_proc_U0_ap_ready;
wire    ap_sync_entry_proc_U0_ap_ready;
reg    ap_sync_reg_tpgBackground_U0_ap_ready;
wire    ap_sync_tpgBackground_U0_ap_ready;
wire   [0:0] start_for_tpgForeground_U0_din;
wire    start_for_tpgForeground_U0_full_n;
wire   [0:0] start_for_tpgForeground_U0_dout;
wire    start_for_tpgForeground_U0_empty_n;
wire   [0:0] start_for_MultiPixStream2AXIvideo_U0_din;
wire    start_for_MultiPixStream2AXIvideo_U0_full_n;
wire   [0:0] start_for_MultiPixStream2AXIvideo_U0_dout;
wire    start_for_MultiPixStream2AXIvideo_U0_empty_n;
wire    ap_ce_reg;

// power-on initialization
initial begin
#0 ap_sync_reg_entry_proc_U0_ap_ready = 1'b0;
#0 ap_sync_reg_tpgBackground_U0_ap_ready = 1'b0;
end

bd_quad_vmix_dp_v_tpg_3_0_entry_proc entry_proc_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(entry_proc_U0_ap_start),
    .start_full_n(entry_proc_U0_start_full_n),
    .ap_done(entry_proc_U0_ap_done),
    .ap_continue(entry_proc_U0_ap_continue),
    .ap_idle(entry_proc_U0_ap_idle),
    .ap_ready(entry_proc_U0_ap_ready),
    .start_out(entry_proc_U0_start_out),
    .start_write(entry_proc_U0_start_write),
    .field_id_val8(field_id_val8),
    .field_id_val8_c_din(entry_proc_U0_field_id_val8_c_din),
    .field_id_val8_c_full_n(field_id_val8_c_full_n),
    .field_id_val8_c_write(entry_proc_U0_field_id_val8_c_write),
    .field_id_val8_c_num_data_valid(field_id_val8_c_num_data_valid),
    .field_id_val8_c_fifo_cap(field_id_val8_c_fifo_cap),
    .fid_in_val9(fid_in_val9),
    .fid_in_val9_c_din(entry_proc_U0_fid_in_val9_c_din),
    .fid_in_val9_c_full_n(fid_in_val9_c_full_n),
    .fid_in_val9_c_write(entry_proc_U0_fid_in_val9_c_write),
    .fid_in_val9_c_num_data_valid(fid_in_val9_c_num_data_valid),
    .fid_in_val9_c_fifo_cap(fid_in_val9_c_fifo_cap),
    .ovrlayId_val11(ovrlayId_val11),
    .ovrlayId_val11_c_din(entry_proc_U0_ovrlayId_val11_c_din),
    .ovrlayId_val11_c_full_n(ovrlayId_val11_c_full_n),
    .ovrlayId_val11_c_write(entry_proc_U0_ovrlayId_val11_c_write),
    .ovrlayId_val11_c_num_data_valid(ovrlayId_val11_c_num_data_valid),
    .ovrlayId_val11_c_fifo_cap(ovrlayId_val11_c_fifo_cap),
    .maskId_val12(maskId_val12),
    .maskId_val12_c_din(entry_proc_U0_maskId_val12_c_din),
    .maskId_val12_c_full_n(maskId_val12_c_full_n),
    .maskId_val12_c_write(entry_proc_U0_maskId_val12_c_write),
    .maskId_val12_c_num_data_valid(maskId_val12_c_num_data_valid),
    .maskId_val12_c_fifo_cap(maskId_val12_c_fifo_cap),
    .crossHairX_val18(crossHairX_val18),
    .crossHairX_val18_c_din(entry_proc_U0_crossHairX_val18_c_din),
    .crossHairX_val18_c_full_n(crossHairX_val18_c_full_n),
    .crossHairX_val18_c_write(entry_proc_U0_crossHairX_val18_c_write),
    .crossHairX_val18_c_num_data_valid(crossHairX_val18_c_num_data_valid),
    .crossHairX_val18_c_fifo_cap(crossHairX_val18_c_fifo_cap),
    .crossHairY_val19(crossHairY_val19),
    .crossHairY_val19_c_din(entry_proc_U0_crossHairY_val19_c_din),
    .crossHairY_val19_c_full_n(crossHairY_val19_c_full_n),
    .crossHairY_val19_c_write(entry_proc_U0_crossHairY_val19_c_write),
    .crossHairY_val19_c_num_data_valid(crossHairY_val19_c_num_data_valid),
    .crossHairY_val19_c_fifo_cap(crossHairY_val19_c_fifo_cap),
    .boxSize_val20(boxSize_val20),
    .boxSize_val20_c_din(entry_proc_U0_boxSize_val20_c_din),
    .boxSize_val20_c_full_n(boxSize_val20_c_full_n),
    .boxSize_val20_c_write(entry_proc_U0_boxSize_val20_c_write),
    .boxSize_val20_c_num_data_valid(boxSize_val20_c_num_data_valid),
    .boxSize_val20_c_fifo_cap(boxSize_val20_c_fifo_cap),
    .boxColorR_val21(boxColorR_val21),
    .boxColorR_val21_c_din(entry_proc_U0_boxColorR_val21_c_din),
    .boxColorR_val21_c_full_n(boxColorR_val21_c_full_n),
    .boxColorR_val21_c_write(entry_proc_U0_boxColorR_val21_c_write),
    .boxColorR_val21_c_num_data_valid(boxColorR_val21_c_num_data_valid),
    .boxColorR_val21_c_fifo_cap(boxColorR_val21_c_fifo_cap),
    .boxColorG_val22(boxColorG_val22),
    .boxColorG_val22_c_din(entry_proc_U0_boxColorG_val22_c_din),
    .boxColorG_val22_c_full_n(boxColorG_val22_c_full_n),
    .boxColorG_val22_c_write(entry_proc_U0_boxColorG_val22_c_write),
    .boxColorG_val22_c_num_data_valid(boxColorG_val22_c_num_data_valid),
    .boxColorG_val22_c_fifo_cap(boxColorG_val22_c_fifo_cap),
    .boxColorB_val23(boxColorB_val23),
    .boxColorB_val23_c_din(entry_proc_U0_boxColorB_val23_c_din),
    .boxColorB_val23_c_full_n(boxColorB_val23_c_full_n),
    .boxColorB_val23_c_write(entry_proc_U0_boxColorB_val23_c_write),
    .boxColorB_val23_c_num_data_valid(boxColorB_val23_c_num_data_valid),
    .boxColorB_val23_c_fifo_cap(boxColorB_val23_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_tpgBackground tpgBackground_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(tpgBackground_U0_ap_start),
    .ap_done(tpgBackground_U0_ap_done),
    .ap_continue(tpgBackground_U0_ap_continue),
    .ap_idle(tpgBackground_U0_ap_idle),
    .ap_ready(tpgBackground_U0_ap_ready),
    .height_val(height_val4),
    .width_val(width_val7),
    .patternId_val(bckgndId_val10),
    .motionSpeed_val(motionSpeed_val14),
    .colorFormat_val(colorFormat_val17),
    .bckgndYUV_din(tpgBackground_U0_bckgndYUV_din),
    .bckgndYUV_full_n(bckgndYUV_full_n),
    .bckgndYUV_write(tpgBackground_U0_bckgndYUV_write),
    .bckgndYUV_num_data_valid(tpgBackground_U0_bckgndYUV_num_data_valid),
    .bckgndYUV_fifo_cap(tpgBackground_U0_bckgndYUV_fifo_cap),
    .height_val4_c3_din(tpgBackground_U0_height_val4_c3_din),
    .height_val4_c3_full_n(height_val4_c3_full_n),
    .height_val4_c3_write(tpgBackground_U0_height_val4_c3_write),
    .height_val4_c3_num_data_valid(height_val4_c3_num_data_valid),
    .height_val4_c3_fifo_cap(height_val4_c3_fifo_cap),
    .width_val7_c4_din(tpgBackground_U0_width_val7_c4_din),
    .width_val7_c4_full_n(width_val7_c4_full_n),
    .width_val7_c4_write(tpgBackground_U0_width_val7_c4_write),
    .width_val7_c4_num_data_valid(width_val7_c4_num_data_valid),
    .width_val7_c4_fifo_cap(width_val7_c4_fifo_cap),
    .motionSpeed_val14_c_din(tpgBackground_U0_motionSpeed_val14_c_din),
    .motionSpeed_val14_c_full_n(motionSpeed_val14_c_full_n),
    .motionSpeed_val14_c_write(tpgBackground_U0_motionSpeed_val14_c_write),
    .motionSpeed_val14_c_num_data_valid(motionSpeed_val14_c_num_data_valid),
    .motionSpeed_val14_c_fifo_cap(motionSpeed_val14_c_fifo_cap),
    .colorFormat_val17_c5_din(tpgBackground_U0_colorFormat_val17_c5_din),
    .colorFormat_val17_c5_full_n(colorFormat_val17_c5_full_n),
    .colorFormat_val17_c5_write(tpgBackground_U0_colorFormat_val17_c5_write),
    .colorFormat_val17_c5_num_data_valid(colorFormat_val17_c5_num_data_valid),
    .colorFormat_val17_c5_fifo_cap(colorFormat_val17_c5_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_tpgForeground tpgForeground_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(tpgForeground_U0_ap_start),
    .ap_done(tpgForeground_U0_ap_done),
    .ap_continue(tpgForeground_U0_ap_continue),
    .ap_idle(tpgForeground_U0_ap_idle),
    .ap_ready(tpgForeground_U0_ap_ready),
    .bckgndYUV_dout(bckgndYUV_dout),
    .bckgndYUV_empty_n(bckgndYUV_empty_n),
    .bckgndYUV_read(tpgForeground_U0_bckgndYUV_read),
    .bckgndYUV_num_data_valid(bckgndYUV_num_data_valid),
    .bckgndYUV_fifo_cap(bckgndYUV_fifo_cap),
    .height_val_dout(height_val4_c3_dout),
    .height_val_empty_n(height_val4_c3_empty_n),
    .height_val_read(tpgForeground_U0_height_val_read),
    .height_val_num_data_valid(height_val4_c3_num_data_valid),
    .height_val_fifo_cap(height_val4_c3_fifo_cap),
    .width_val_dout(width_val7_c4_dout),
    .width_val_empty_n(width_val7_c4_empty_n),
    .width_val_read(tpgForeground_U0_width_val_read),
    .width_val_num_data_valid(width_val7_c4_num_data_valid),
    .width_val_fifo_cap(width_val7_c4_fifo_cap),
    .patternId_val_dout(ovrlayId_val11_c_dout),
    .patternId_val_empty_n(ovrlayId_val11_c_empty_n),
    .patternId_val_read(tpgForeground_U0_patternId_val_read),
    .patternId_val_num_data_valid(ovrlayId_val11_c_num_data_valid),
    .patternId_val_fifo_cap(ovrlayId_val11_c_fifo_cap),
    .maskId_val_dout(maskId_val12_c_dout),
    .maskId_val_empty_n(maskId_val12_c_empty_n),
    .maskId_val_read(tpgForeground_U0_maskId_val_read),
    .maskId_val_num_data_valid(maskId_val12_c_num_data_valid),
    .maskId_val_fifo_cap(maskId_val12_c_fifo_cap),
    .colorFormat_val_dout(colorFormat_val17_c5_dout),
    .colorFormat_val_empty_n(colorFormat_val17_c5_empty_n),
    .colorFormat_val_read(tpgForeground_U0_colorFormat_val_read),
    .colorFormat_val_num_data_valid(colorFormat_val17_c5_num_data_valid),
    .colorFormat_val_fifo_cap(colorFormat_val17_c5_fifo_cap),
    .crossHairX_val_dout(crossHairX_val18_c_dout),
    .crossHairX_val_empty_n(crossHairX_val18_c_empty_n),
    .crossHairX_val_read(tpgForeground_U0_crossHairX_val_read),
    .crossHairX_val_num_data_valid(crossHairX_val18_c_num_data_valid),
    .crossHairX_val_fifo_cap(crossHairX_val18_c_fifo_cap),
    .crossHairY_val_dout(crossHairY_val19_c_dout),
    .crossHairY_val_empty_n(crossHairY_val19_c_empty_n),
    .crossHairY_val_read(tpgForeground_U0_crossHairY_val_read),
    .crossHairY_val_num_data_valid(crossHairY_val19_c_num_data_valid),
    .crossHairY_val_fifo_cap(crossHairY_val19_c_fifo_cap),
    .boxSize_val_dout(boxSize_val20_c_dout),
    .boxSize_val_empty_n(boxSize_val20_c_empty_n),
    .boxSize_val_read(tpgForeground_U0_boxSize_val_read),
    .boxSize_val_num_data_valid(boxSize_val20_c_num_data_valid),
    .boxSize_val_fifo_cap(boxSize_val20_c_fifo_cap),
    .boxColorR_val_dout(boxColorR_val21_c_dout),
    .boxColorR_val_empty_n(boxColorR_val21_c_empty_n),
    .boxColorR_val_read(tpgForeground_U0_boxColorR_val_read),
    .boxColorR_val_num_data_valid(boxColorR_val21_c_num_data_valid),
    .boxColorR_val_fifo_cap(boxColorR_val21_c_fifo_cap),
    .boxColorG_val_dout(boxColorG_val22_c_dout),
    .boxColorG_val_empty_n(boxColorG_val22_c_empty_n),
    .boxColorG_val_read(tpgForeground_U0_boxColorG_val_read),
    .boxColorG_val_num_data_valid(boxColorG_val22_c_num_data_valid),
    .boxColorG_val_fifo_cap(boxColorG_val22_c_fifo_cap),
    .boxColorB_val_dout(boxColorB_val23_c_dout),
    .boxColorB_val_empty_n(boxColorB_val23_c_empty_n),
    .boxColorB_val_read(tpgForeground_U0_boxColorB_val_read),
    .boxColorB_val_num_data_valid(boxColorB_val23_c_num_data_valid),
    .boxColorB_val_fifo_cap(boxColorB_val23_c_fifo_cap),
    .motionSpeed_val_dout(motionSpeed_val14_c_dout),
    .motionSpeed_val_empty_n(motionSpeed_val14_c_empty_n),
    .motionSpeed_val_read(tpgForeground_U0_motionSpeed_val_read),
    .motionSpeed_val_num_data_valid(motionSpeed_val14_c_num_data_valid),
    .motionSpeed_val_fifo_cap(motionSpeed_val14_c_fifo_cap),
    .ovrlayYUV_din(tpgForeground_U0_ovrlayYUV_din),
    .ovrlayYUV_full_n(ovrlayYUV_full_n),
    .ovrlayYUV_write(tpgForeground_U0_ovrlayYUV_write),
    .ovrlayYUV_num_data_valid(tpgForeground_U0_ovrlayYUV_num_data_valid),
    .ovrlayYUV_fifo_cap(tpgForeground_U0_ovrlayYUV_fifo_cap),
    .height_val4_c_din(tpgForeground_U0_height_val4_c_din),
    .height_val4_c_full_n(height_val4_c_full_n),
    .height_val4_c_write(tpgForeground_U0_height_val4_c_write),
    .height_val4_c_num_data_valid(height_val4_c_num_data_valid),
    .height_val4_c_fifo_cap(height_val4_c_fifo_cap),
    .width_val7_c_din(tpgForeground_U0_width_val7_c_din),
    .width_val7_c_full_n(width_val7_c_full_n),
    .width_val7_c_write(tpgForeground_U0_width_val7_c_write),
    .width_val7_c_num_data_valid(width_val7_c_num_data_valid),
    .width_val7_c_fifo_cap(width_val7_c_fifo_cap),
    .colorFormat_val17_c_din(tpgForeground_U0_colorFormat_val17_c_din),
    .colorFormat_val17_c_full_n(colorFormat_val17_c_full_n),
    .colorFormat_val17_c_write(tpgForeground_U0_colorFormat_val17_c_write),
    .colorFormat_val17_c_num_data_valid(colorFormat_val17_c_num_data_valid),
    .colorFormat_val17_c_fifo_cap(colorFormat_val17_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_MultiPixStream2AXIvideo MultiPixStream2AXIvideo_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(MultiPixStream2AXIvideo_U0_ap_start),
    .ap_done(MultiPixStream2AXIvideo_U0_ap_done),
    .ap_continue(MultiPixStream2AXIvideo_U0_ap_continue),
    .ap_idle(MultiPixStream2AXIvideo_U0_ap_idle),
    .ap_ready(MultiPixStream2AXIvideo_U0_ap_ready),
    .ovrlayYUV_dout(ovrlayYUV_dout),
    .ovrlayYUV_empty_n(ovrlayYUV_empty_n),
    .ovrlayYUV_read(MultiPixStream2AXIvideo_U0_ovrlayYUV_read),
    .ovrlayYUV_num_data_valid(ovrlayYUV_num_data_valid),
    .ovrlayYUV_fifo_cap(ovrlayYUV_fifo_cap),
    .m_axis_video_TDATA(MultiPixStream2AXIvideo_U0_m_axis_video_TDATA),
    .m_axis_video_TVALID(MultiPixStream2AXIvideo_U0_m_axis_video_TVALID),
    .m_axis_video_TREADY(m_axis_video_TREADY),
    .m_axis_video_TKEEP(MultiPixStream2AXIvideo_U0_m_axis_video_TKEEP),
    .m_axis_video_TSTRB(MultiPixStream2AXIvideo_U0_m_axis_video_TSTRB),
    .m_axis_video_TUSER(MultiPixStream2AXIvideo_U0_m_axis_video_TUSER),
    .m_axis_video_TLAST(MultiPixStream2AXIvideo_U0_m_axis_video_TLAST),
    .m_axis_video_TID(MultiPixStream2AXIvideo_U0_m_axis_video_TID),
    .m_axis_video_TDEST(MultiPixStream2AXIvideo_U0_m_axis_video_TDEST),
    .height_val4_dout(height_val4_c_dout),
    .height_val4_empty_n(height_val4_c_empty_n),
    .height_val4_read(MultiPixStream2AXIvideo_U0_height_val4_read),
    .height_val4_num_data_valid(height_val4_c_num_data_valid),
    .height_val4_fifo_cap(height_val4_c_fifo_cap),
    .width_val7_dout(width_val7_c_dout),
    .width_val7_empty_n(width_val7_c_empty_n),
    .width_val7_read(MultiPixStream2AXIvideo_U0_width_val7_read),
    .width_val7_num_data_valid(width_val7_c_num_data_valid),
    .width_val7_fifo_cap(width_val7_c_fifo_cap),
    .colorFormat_val17_dout(colorFormat_val17_c_dout),
    .colorFormat_val17_empty_n(colorFormat_val17_c_empty_n),
    .colorFormat_val17_read(MultiPixStream2AXIvideo_U0_colorFormat_val17_read),
    .colorFormat_val17_num_data_valid(colorFormat_val17_c_num_data_valid),
    .colorFormat_val17_fifo_cap(colorFormat_val17_c_fifo_cap),
    .fid_in_val9_dout(fid_in_val9_c_dout),
    .fid_in_val9_empty_n(fid_in_val9_c_empty_n),
    .fid_in_val9_read(MultiPixStream2AXIvideo_U0_fid_in_val9_read),
    .fid_in_val9_num_data_valid(fid_in_val9_c_num_data_valid),
    .fid_in_val9_fifo_cap(fid_in_val9_c_fifo_cap),
    .fid(MultiPixStream2AXIvideo_U0_fid),
    .fid_ap_vld(MultiPixStream2AXIvideo_U0_fid_ap_vld),
    .field_id_val8_dout(field_id_val8_c_dout),
    .field_id_val8_empty_n(field_id_val8_c_empty_n),
    .field_id_val8_read(MultiPixStream2AXIvideo_U0_field_id_val8_read),
    .field_id_val8_num_data_valid(field_id_val8_c_num_data_valid),
    .field_id_val8_fifo_cap(field_id_val8_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w16_d4_S field_id_val8_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_field_id_val8_c_din),
    .if_full_n(field_id_val8_c_full_n),
    .if_write(entry_proc_U0_field_id_val8_c_write),
    .if_dout(field_id_val8_c_dout),
    .if_empty_n(field_id_val8_c_empty_n),
    .if_read(MultiPixStream2AXIvideo_U0_field_id_val8_read),
    .if_num_data_valid(field_id_val8_c_num_data_valid),
    .if_fifo_cap(field_id_val8_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w1_d4_S fid_in_val9_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_fid_in_val9_c_din),
    .if_full_n(fid_in_val9_c_full_n),
    .if_write(entry_proc_U0_fid_in_val9_c_write),
    .if_dout(fid_in_val9_c_dout),
    .if_empty_n(fid_in_val9_c_empty_n),
    .if_read(MultiPixStream2AXIvideo_U0_fid_in_val9_read),
    .if_num_data_valid(fid_in_val9_c_num_data_valid),
    .if_fifo_cap(fid_in_val9_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w8_d3_S ovrlayId_val11_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_ovrlayId_val11_c_din),
    .if_full_n(ovrlayId_val11_c_full_n),
    .if_write(entry_proc_U0_ovrlayId_val11_c_write),
    .if_dout(ovrlayId_val11_c_dout),
    .if_empty_n(ovrlayId_val11_c_empty_n),
    .if_read(tpgForeground_U0_patternId_val_read),
    .if_num_data_valid(ovrlayId_val11_c_num_data_valid),
    .if_fifo_cap(ovrlayId_val11_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w8_d3_S maskId_val12_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_maskId_val12_c_din),
    .if_full_n(maskId_val12_c_full_n),
    .if_write(entry_proc_U0_maskId_val12_c_write),
    .if_dout(maskId_val12_c_dout),
    .if_empty_n(maskId_val12_c_empty_n),
    .if_read(tpgForeground_U0_maskId_val_read),
    .if_num_data_valid(maskId_val12_c_num_data_valid),
    .if_fifo_cap(maskId_val12_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w16_d3_S crossHairX_val18_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_crossHairX_val18_c_din),
    .if_full_n(crossHairX_val18_c_full_n),
    .if_write(entry_proc_U0_crossHairX_val18_c_write),
    .if_dout(crossHairX_val18_c_dout),
    .if_empty_n(crossHairX_val18_c_empty_n),
    .if_read(tpgForeground_U0_crossHairX_val_read),
    .if_num_data_valid(crossHairX_val18_c_num_data_valid),
    .if_fifo_cap(crossHairX_val18_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w16_d3_S crossHairY_val19_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_crossHairY_val19_c_din),
    .if_full_n(crossHairY_val19_c_full_n),
    .if_write(entry_proc_U0_crossHairY_val19_c_write),
    .if_dout(crossHairY_val19_c_dout),
    .if_empty_n(crossHairY_val19_c_empty_n),
    .if_read(tpgForeground_U0_crossHairY_val_read),
    .if_num_data_valid(crossHairY_val19_c_num_data_valid),
    .if_fifo_cap(crossHairY_val19_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w16_d3_S boxSize_val20_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_boxSize_val20_c_din),
    .if_full_n(boxSize_val20_c_full_n),
    .if_write(entry_proc_U0_boxSize_val20_c_write),
    .if_dout(boxSize_val20_c_dout),
    .if_empty_n(boxSize_val20_c_empty_n),
    .if_read(tpgForeground_U0_boxSize_val_read),
    .if_num_data_valid(boxSize_val20_c_num_data_valid),
    .if_fifo_cap(boxSize_val20_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w8_d3_S boxColorR_val21_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_boxColorR_val21_c_din),
    .if_full_n(boxColorR_val21_c_full_n),
    .if_write(entry_proc_U0_boxColorR_val21_c_write),
    .if_dout(boxColorR_val21_c_dout),
    .if_empty_n(boxColorR_val21_c_empty_n),
    .if_read(tpgForeground_U0_boxColorR_val_read),
    .if_num_data_valid(boxColorR_val21_c_num_data_valid),
    .if_fifo_cap(boxColorR_val21_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w8_d3_S boxColorG_val22_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_boxColorG_val22_c_din),
    .if_full_n(boxColorG_val22_c_full_n),
    .if_write(entry_proc_U0_boxColorG_val22_c_write),
    .if_dout(boxColorG_val22_c_dout),
    .if_empty_n(boxColorG_val22_c_empty_n),
    .if_read(tpgForeground_U0_boxColorG_val_read),
    .if_num_data_valid(boxColorG_val22_c_num_data_valid),
    .if_fifo_cap(boxColorG_val22_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w8_d3_S boxColorB_val23_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(entry_proc_U0_boxColorB_val23_c_din),
    .if_full_n(boxColorB_val23_c_full_n),
    .if_write(entry_proc_U0_boxColorB_val23_c_write),
    .if_dout(boxColorB_val23_c_dout),
    .if_empty_n(boxColorB_val23_c_empty_n),
    .if_read(tpgForeground_U0_boxColorB_val_read),
    .if_num_data_valid(boxColorB_val23_c_num_data_valid),
    .if_fifo_cap(boxColorB_val23_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w48_d16_S bckgndYUV_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgBackground_U0_bckgndYUV_din),
    .if_full_n(bckgndYUV_full_n),
    .if_write(tpgBackground_U0_bckgndYUV_write),
    .if_dout(bckgndYUV_dout),
    .if_empty_n(bckgndYUV_empty_n),
    .if_read(tpgForeground_U0_bckgndYUV_read),
    .if_num_data_valid(bckgndYUV_num_data_valid),
    .if_fifo_cap(bckgndYUV_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w16_d2_S height_val4_c3_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgBackground_U0_height_val4_c3_din),
    .if_full_n(height_val4_c3_full_n),
    .if_write(tpgBackground_U0_height_val4_c3_write),
    .if_dout(height_val4_c3_dout),
    .if_empty_n(height_val4_c3_empty_n),
    .if_read(tpgForeground_U0_height_val_read),
    .if_num_data_valid(height_val4_c3_num_data_valid),
    .if_fifo_cap(height_val4_c3_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w16_d2_S width_val7_c4_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgBackground_U0_width_val7_c4_din),
    .if_full_n(width_val7_c4_full_n),
    .if_write(tpgBackground_U0_width_val7_c4_write),
    .if_dout(width_val7_c4_dout),
    .if_empty_n(width_val7_c4_empty_n),
    .if_read(tpgForeground_U0_width_val_read),
    .if_num_data_valid(width_val7_c4_num_data_valid),
    .if_fifo_cap(width_val7_c4_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w8_d2_S motionSpeed_val14_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgBackground_U0_motionSpeed_val14_c_din),
    .if_full_n(motionSpeed_val14_c_full_n),
    .if_write(tpgBackground_U0_motionSpeed_val14_c_write),
    .if_dout(motionSpeed_val14_c_dout),
    .if_empty_n(motionSpeed_val14_c_empty_n),
    .if_read(tpgForeground_U0_motionSpeed_val_read),
    .if_num_data_valid(motionSpeed_val14_c_num_data_valid),
    .if_fifo_cap(motionSpeed_val14_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w8_d2_S colorFormat_val17_c5_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgBackground_U0_colorFormat_val17_c5_din),
    .if_full_n(colorFormat_val17_c5_full_n),
    .if_write(tpgBackground_U0_colorFormat_val17_c5_write),
    .if_dout(colorFormat_val17_c5_dout),
    .if_empty_n(colorFormat_val17_c5_empty_n),
    .if_read(tpgForeground_U0_colorFormat_val_read),
    .if_num_data_valid(colorFormat_val17_c5_num_data_valid),
    .if_fifo_cap(colorFormat_val17_c5_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w48_d16_S ovrlayYUV_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgForeground_U0_ovrlayYUV_din),
    .if_full_n(ovrlayYUV_full_n),
    .if_write(tpgForeground_U0_ovrlayYUV_write),
    .if_dout(ovrlayYUV_dout),
    .if_empty_n(ovrlayYUV_empty_n),
    .if_read(MultiPixStream2AXIvideo_U0_ovrlayYUV_read),
    .if_num_data_valid(ovrlayYUV_num_data_valid),
    .if_fifo_cap(ovrlayYUV_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w11_d2_S height_val4_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgForeground_U0_height_val4_c_din),
    .if_full_n(height_val4_c_full_n),
    .if_write(tpgForeground_U0_height_val4_c_write),
    .if_dout(height_val4_c_dout),
    .if_empty_n(height_val4_c_empty_n),
    .if_read(MultiPixStream2AXIvideo_U0_height_val4_read),
    .if_num_data_valid(height_val4_c_num_data_valid),
    .if_fifo_cap(height_val4_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w11_d2_S width_val7_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgForeground_U0_width_val7_c_din),
    .if_full_n(width_val7_c_full_n),
    .if_write(tpgForeground_U0_width_val7_c_write),
    .if_dout(width_val7_c_dout),
    .if_empty_n(width_val7_c_empty_n),
    .if_read(MultiPixStream2AXIvideo_U0_width_val7_read),
    .if_num_data_valid(width_val7_c_num_data_valid),
    .if_fifo_cap(width_val7_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_fifo_w8_d2_S colorFormat_val17_c_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(tpgForeground_U0_colorFormat_val17_c_din),
    .if_full_n(colorFormat_val17_c_full_n),
    .if_write(tpgForeground_U0_colorFormat_val17_c_write),
    .if_dout(colorFormat_val17_c_dout),
    .if_empty_n(colorFormat_val17_c_empty_n),
    .if_read(MultiPixStream2AXIvideo_U0_colorFormat_val17_read),
    .if_num_data_valid(colorFormat_val17_c_num_data_valid),
    .if_fifo_cap(colorFormat_val17_c_fifo_cap)
);

bd_quad_vmix_dp_v_tpg_3_0_start_for_tpgForeground_U0 start_for_tpgForeground_U0_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(start_for_tpgForeground_U0_din),
    .if_full_n(start_for_tpgForeground_U0_full_n),
    .if_write(entry_proc_U0_start_write),
    .if_dout(start_for_tpgForeground_U0_dout),
    .if_empty_n(start_for_tpgForeground_U0_empty_n),
    .if_read(tpgForeground_U0_ap_ready)
);

bd_quad_vmix_dp_v_tpg_3_0_start_for_MultiPixStream2AXIvideo_U0 start_for_MultiPixStream2AXIvideo_U0_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(start_for_MultiPixStream2AXIvideo_U0_din),
    .if_full_n(start_for_MultiPixStream2AXIvideo_U0_full_n),
    .if_write(entry_proc_U0_start_write),
    .if_dout(start_for_MultiPixStream2AXIvideo_U0_dout),
    .if_empty_n(start_for_MultiPixStream2AXIvideo_U0_empty_n),
    .if_read(MultiPixStream2AXIvideo_U0_ap_ready)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_sync_reg_entry_proc_U0_ap_ready <= 1'b0;
    end else begin
        if (((ap_sync_ready & ap_start) == 1'b1)) begin
            ap_sync_reg_entry_proc_U0_ap_ready <= 1'b0;
        end else begin
            ap_sync_reg_entry_proc_U0_ap_ready <= ap_sync_entry_proc_U0_ap_ready;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_sync_reg_tpgBackground_U0_ap_ready <= 1'b0;
    end else begin
        if (((ap_sync_ready & ap_start) == 1'b1)) begin
            ap_sync_reg_tpgBackground_U0_ap_ready <= 1'b0;
        end else begin
            ap_sync_reg_tpgBackground_U0_ap_ready <= ap_sync_tpgBackground_U0_ap_ready;
        end
    end
end

assign MultiPixStream2AXIvideo_U0_ap_continue = ap_continue;

assign MultiPixStream2AXIvideo_U0_ap_start = start_for_MultiPixStream2AXIvideo_U0_empty_n;

assign ap_done = MultiPixStream2AXIvideo_U0_ap_done;

assign ap_idle = (tpgForeground_U0_ap_idle & tpgBackground_U0_ap_idle & entry_proc_U0_ap_idle & MultiPixStream2AXIvideo_U0_ap_idle);

assign ap_ready = ap_sync_ready;

assign ap_sync_entry_proc_U0_ap_ready = (entry_proc_U0_ap_ready | ap_sync_reg_entry_proc_U0_ap_ready);

assign ap_sync_ready = (ap_sync_tpgBackground_U0_ap_ready & ap_sync_entry_proc_U0_ap_ready);

assign ap_sync_tpgBackground_U0_ap_ready = (tpgBackground_U0_ap_ready | ap_sync_reg_tpgBackground_U0_ap_ready);

assign entry_proc_U0_ap_continue = 1'b1;

assign entry_proc_U0_ap_start = ((ap_sync_reg_entry_proc_U0_ap_ready ^ 1'b1) & ap_start);

assign entry_proc_U0_start_full_n = (start_for_tpgForeground_U0_full_n & start_for_MultiPixStream2AXIvideo_U0_full_n);

assign fid = MultiPixStream2AXIvideo_U0_fid;

assign fid_ap_vld = MultiPixStream2AXIvideo_U0_fid_ap_vld;

assign m_axis_video_TDATA = MultiPixStream2AXIvideo_U0_m_axis_video_TDATA;

assign m_axis_video_TDEST = MultiPixStream2AXIvideo_U0_m_axis_video_TDEST;

assign m_axis_video_TID = MultiPixStream2AXIvideo_U0_m_axis_video_TID;

assign m_axis_video_TKEEP = MultiPixStream2AXIvideo_U0_m_axis_video_TKEEP;

assign m_axis_video_TLAST = MultiPixStream2AXIvideo_U0_m_axis_video_TLAST;

assign m_axis_video_TSTRB = MultiPixStream2AXIvideo_U0_m_axis_video_TSTRB;

assign m_axis_video_TUSER = MultiPixStream2AXIvideo_U0_m_axis_video_TUSER;

assign m_axis_video_TVALID = MultiPixStream2AXIvideo_U0_m_axis_video_TVALID;

assign start_for_MultiPixStream2AXIvideo_U0_din = 1'b1;

assign start_for_tpgForeground_U0_din = 1'b1;

assign tpgBackground_U0_ap_continue = 1'b1;

assign tpgBackground_U0_ap_start = ((ap_sync_reg_tpgBackground_U0_ap_ready ^ 1'b1) & ap_start);

assign tpgBackground_U0_bckgndYUV_fifo_cap = bckgndYUV_fifo_cap;

assign tpgBackground_U0_bckgndYUV_num_data_valid = bckgndYUV_num_data_valid;

assign tpgForeground_U0_ap_continue = 1'b1;

assign tpgForeground_U0_ap_start = start_for_tpgForeground_U0_empty_n;

assign tpgForeground_U0_ovrlayYUV_fifo_cap = ovrlayYUV_fifo_cap;

assign tpgForeground_U0_ovrlayYUV_num_data_valid = ovrlayYUV_num_data_valid;

endmodule //bd_quad_vmix_dp_v_tpg_3_0_v_tpgHlsDataFlow
